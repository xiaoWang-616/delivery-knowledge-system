...
- 在交付结果页用交付总览聚合任务队列、review、命令验收、系统验收标准、知识沉淀和总验收状态。
...
- 本地 runner 根据任务包读取接口文档、Demo / 设计图和 PRD，生成给 AI 开发前使用的上下文包。
...
- 本地 runner 支持系统验收标准生成，根据需求、接口资料、Demo 和项目扫描结果整理验收项，并写入知识库报告。
...
- 当前阶段是什么。
- 下一步该派任务、等报告、review、修复、命令验收、系统验收标准、知识沉淀还是总验收。
...
交付结果页顶部的“交付总览”会把任务队列、最近一次系统 review、命令验收、系统验收标准、知识沉淀和总验收聚合成状态卡，并显示下一步建议。它只读取当前页面已有状态，不直接写真实项目。
...
点击“生成总验收”后，runner 会读取任务队列、review 文件、命令验收结果、系统验收标准、知识库写入结果和运行问题池，生成：
...
- `success`：任务队列全部完成，review、命令验收、系统验收标准和知识沉淀都有记录。
...
- 系统验收标准生成，根据需求、接口资料、Demo 和项目扫描结果整理验收项
- 当前任务派发、报告回填、轻量 review、git 改动范围检查和修复任务生成
- 总验收与规则沉淀候选生成
- 知识库写入
- 自动生成 `08-项目实例/{项目名}` 记录
- 自动生成模块上下文包、接口能力矩阵、样式参考摘要、工程风险点和 AI 执行提示词
...
- 任务全部完成后，依次执行命令验收、系统验收标准生成、知识库写入和总验收。
- 遇到 assigned、blocked、命令失败、验收失败、缺少授权时停止，并返回原因和下一步。
...
任务包不再要求填写页面 URL 或点测关键词。点击“生成验收标准”后，runner 会根据这些资料自动整理验收项：

- 需求说明里的模块目标、边界和不要做事项。
- 接口文档里的路径、参数、响应字段、分页和错误码。
- Demo / 设计图里的首屏布局、表格密度、按钮状态、空态和错误态。
- 项目扫描出的脚本、目录结构和规则文件。
- 任务队列中每个 task 的 acceptance。

结果会显示在交付结果页，并写入：

```text
08-项目实例/{项目名}/{模块名}/系统验收标准-{时间戳}.auto.md
```

浏览器自动识别页面入口、截图和视觉回归属于后续增强能力；当前阶段先把可验收标准稳定沉淀下来。
...
运行状态快照包含任务包、步骤状态、运行日志、项目画像、命令验收结果、系统验收标准、知识库写入结果和问题清单。它是系统内部的调试和复盘资料，正常用户不需要手动保存、恢复或管理历史记录。`项目历史辅助文件/` 已写入 `.gitignore`，不会提交 Git，也不会进入用户真实项目。
